...
class TrajDataEmbedding(nn.Module):
    def __init__(self, c_in, d_model, embed_type='fixed', freq='s', dropout=0.1):
        super(TrajDataEmbedding, self).__init__()
        #self.global_attn_layer = GlobalAttnLayer(c_in,c_in,8)
        self.embedding_transfer = nn.Linear(c_in, d_model)
        self.temporal_embedding = TemporalEmbedding(d_model=d_model, embed_type=embed_type,
                                                    freq=freq) if embed_type != 'timeF' else TimeFeatureEmbedding(
                                                    d_model=d_model, embed_type=embed_type, freq=freq)
        self.dropout = nn.Dropout(p=dropout)
    ...
    def forward(self, x, x_mark):
        #x是static+dynamic
        # print("@@@@@@@@@@@@@@@@@@@@", x.shape, x, self.embedding_transfer)
        if x_mark is None:
            x = self.embedding_transfer(x)
        else:
            x = self.embedding_transfer(x) + self.temporal_embedding(x_mark) 
        return x
