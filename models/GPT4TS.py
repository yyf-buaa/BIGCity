...
class ST_Tokenizer(nn.Module):
    def __init__(self, city):
        ...
        # dynamic embedding
        self.dynamic_embedding = torch.from_numpy(np.load('/root/dataset/{}/road_dyna_embedding.npy'.format(city))).float() # N*T*d
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.enc_embedding =TrajDataEmbedding(c_in=256,d_model=configs.d_model,embed_type=configs.embed)
    ...
    def pretrain(self, x_id, x_time, x_mark_enc):
        ...
        print(type(self.gpt2(inputs_embeds=enc_out)))
    ...
    def token_embedding(self, x_id, x_time):
        ...
        print("xid.shape::::::::::::::::::::", x_id.shape)
        B, L = x_id.shape
        print(x_id.dtype, type(x_id),x_id.shape, x_time.dtype, type(x_time), x_time.shape)
        print(x_id, x_time)
        x_enc = road_embedding[x_id, x_time.to(torch.int64)].to(x_id.device)
        return x_enc # B * L * d
