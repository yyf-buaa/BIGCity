...
class StTokenizer(nn.Module):
    ...
    def forward(self, road_id_batch, time_id_batch, time_feature_batch):
        self.device = road_id_batch.device
        print(road_id_batch)
        B, N, M, L = road_id_batch.shape[0], self.road_cnt, self.edge_cnt, args.seq_len
        
        self.static_origin_embedding = self.static_origin_embedding.to(self.device)
        ...
        dynamic_embedding_result = dynamic_embedding[road_id_batch, torch.arange(B).unsqueeze(1).expand(B, L).to(self.device)]
