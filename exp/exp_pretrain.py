...
class Exp_Pretrain(Exp_Basic):
    ...
    def psm_input_time(self, batch_time_features, mask, num_mask):
        ...
        return torch.cat([batch_masked_time_features, special_token], dim=1), mask
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (
            batch_road_id, batch_traj_time_index, batch_time_features, batch_road_time, batch_road_flow) in tqdm(enumerate(train_loader)):
                iter_count += 1
                self.model_optim.zero_grad()
                print('road time:', batch_road_time.shape)
                print('traj_time_index:',batch_traj_time_index)
                B, T = batch_road_id.shape
                ...
                batch_masked_time_features, time_mask = self.psm_input_time(batch_time_features, mask, num_mask)  # mask and psm

                batch_masked_road_id = batch_masked_road_id.long().to(self.device)
                ...
                batch_traj_time_index = batch_traj_time_index.long().to(self.device)

                outputs_road, outputs_time, outputs_flow = self.model(batch_masked_road_id, batch_traj_time_index, batch_masked_time_features)
                ...
                loss_road = self.loss_road_func(torch.flatten(outputs_road, start_dim=0, end_dim=-2),
                                                batch_time_features[time_mask == 0])
                loss_flow = self.loss_flow_func(torch.flatten(outputs_flow, start_dim=0, end_dim=-1),
                                                batch_road_flow[mask == 0])
                logging.info('road loss:',loss_road.item())
                logging.info('time loss:',loss_time.item())
                logging.info('flow loss:',loss_flow.item())
